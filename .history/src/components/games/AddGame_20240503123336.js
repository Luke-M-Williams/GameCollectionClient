...
const AddGame = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem('game_collection_user')
      ...
      : null;
      const formattedDate = new Date(releaseDate).toISOString().slice(0, 10); // Ensures date is in YYYY-MM-DD format
    const gameData = {
      title,
      cover_art: coverArt,
      genre,
      developer,
      release_date: f,
      platforms, // Assuming your backend expects an array of platform IDs
    };
