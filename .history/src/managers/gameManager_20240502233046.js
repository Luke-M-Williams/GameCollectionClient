...
const getGames = async () => {
  ...
  try {
    ...
    const result = await response.json(); // Getting the full JSON response
    if (!Array.isArray(result.data)) {
      throw new Error('Expected games data to be an array, but got something else');
    }
    return result.data; // Now specifically returning the games array
  } catch (error) {
